...
public class Wheel : AbilityData {
	...
	void create(Vector3 pos, Vector3 hitPos, RaycastHit2D hit) {
		...
		spawned.transform.Translate ((spawned.transform.up * spawned.transform.localScale.y) * GetComponent<SpriteRenderer>().sprite.bounds.size.y * 1.3f);
		Vector3 temp = spawned.transform.position;
		temp.z = 0;
		spawned.transform.position = temp;
		foreach (Transform trans in spawned.GetComponentsInChildren<Transform>()) {
			temp = trans.position;
			temp.z = spawned.transform.position.z + 1;
			trans.position = temp;
		}
	}
}